...
export const ScoreStatsBar = memo(function ScoreStatsBar({ 
  ...
  return (
    <div className={`
      ${isMobile ? "" : "bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200/60 dark:border-slate-800 shadow-xl rounded-full px-6 py-3"}
      flex items-center gap-4 transition-all duration-500 animate-in fade-in slide-from-top-4
    `}>
      {/* Experience & Rate Section */}
      {admLevel !== -1 && (
        <div className="flex items-center gap-3 border-r border-slate-200/60 dark:border-slate-800 pr-4 shrink-0">
          <div className="flex flex-col">
            <span className="text-[8px] font-black text-orange-500 dark:text-orange-400 uppercase tracking-tighter leading-none mb-0.5">Rate</span>
            {!hideRate && (
              <div className="flex items-baseline gap-1.5">
                <span className="text-sm font-black text-orange-600 dark:text-orange-400 tabular-nums leading-none">
          ...
          {!hideExp && admLevel !== 0 && (
            <>
              ...
              <div className="flex items-center gap-2">
                <span className="text-[9px] font-black text-blue-500 dark:text-blue-400 uppercase tracking-tighter">EXP</span>
                <span className="text-sm font-black text-blue-600 dark:text-blue-400 tabular-nums leading-none">
        ...
        {(admLevel === undefined || admLevel === -1) && (
          <>
            <div className="flex flex-col items-center">
              ...
              <span className="text-sm font-black text-slate-800 dark:text-white tabular-nums leading-none">
      ...
      <div className={`flex flex-wrap items-center gap-x-4 gap-y-2 flex-1 py-0.5`}>
        {categories.map((cat) => (
          <div key={cat.key} className="flex items-center gap-1.5 group shrink-0">
            <div className={`w-1.5 h-1.5 rounded-full ${cat.color} group-hover:scale-125 transition-transform shadow-xs`} />
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-slate-700 dark:text-slate-200 tabular-nums leading-none mb-0.5">{(stats as any)[cat.key]}</span>
