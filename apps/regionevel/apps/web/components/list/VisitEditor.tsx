    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      <div className="divide-y divide-gray-100">
        {VISIT_CATEGORY_ORDER.map((cat) => {
          ...
          const pct = Math.round(((b?.effectiveCount ?? 0) / cfg.maxCount) * 100);
          const directPct = Math.round(((b?.directCount ?? 0) / cfg.maxCount) * 100);
          ...
          return (
            <div key={cat} className="px-4 py-3">
              <div className="flex items-center justify-between mb-1">
                <div className="flex flex-col">
                  <div className="flex items-center gap-2">
                    ...
                    {(b?.effectiveCount ?? 0) > (b?.directCount ?? 0) && (
                      <span className="text-[10px] text-gray-400 font-medium bg-gray-50 px-1.5 py-0.5 rounded">
                        +{(b?.effectiveCount ?? 0) - (b?.directCount ?? 0)} (from sub-regions)
                      </span>
                    )}
                  </div>
                  ...
                  <span className="text-xs text-gray-500 font-medium">
                    {b?.directCount ?? 0}/{cfg.maxCount} times · {b?.points ?? 0}/{cfg.maxPoints} pts
                  </span>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => upsertVisit(regionId, cat, (b?.directCount ?? 0) - 1)}
                ...
                <div className="flex gap-1">
                  {Array.from({ length: cfg.maxCount }, (_, i) => {
                    const isDirect = i < (b?.directCount ?? 0);
                    const isEffective = i < (b?.effectiveCount ?? 0);
                    return (
                      <button
                        key={i}
                        onClick={() =>
                          upsertVisit(regionId, cat, i < (b?.directCount ?? 0) ? i : i + 1)
                        }
