...
-   `rules.json`: 앱과 웹이 **함께 읽는 값**(도보 환승 반경 등). 손으로 적는 원본입니다.
...
-   **설명**: 앱과 웹이 함께 쓰는 값의 **원본**. 생성기가 만드는 파일이 아니라 손으로 적습니다.
-   **왜 필요한가**: 같은 뜻을 가진 값이 Kotlin 과 TypeScript 에 따로 적혀 있으면 언젠가
    갈라집니다. 실제로 갈라졌습니다 — 이름이 같은 역을 걸어서 잇는 거리가 웹 1.5km,
    앱 1.0km 라 경로 고정물 396개 중 14개가 어긋났습니다.
-   **읽는 것과 맞대는 것**:
    | | 어떻게 | 왜 |
    |---|---|---|
    | `walk_transfer` | 두 클라이언트가 **런타임에 읽습니다** | 그래프 모양을 바꾸는 값이라 어긋나면 바로 경로가 달라집니다 |
    | `transfer_time` · `graph_repair` | 코드에 상수로 두고 **같은지 시험합니다** | 사람이 걷는 속도처럼 데이터와 무관한 값이라 굳이 런타임에 읽을 이유가 없습니다 |
    | `_not_shared` | **나누지 않습니다** | 이름은 비슷해도 뜻이 달라서(앱은 곱하는 값, 웹은 더하는 km) 한 숫자를 같이 쓰면 오히려 틀립니다 |
-   **검증**: `npm run verify:rules` · 앱은 `RailRulesTest`.
-   **값을 믿지 않습니다**: 이 파일은 OTA 로도 들어오므로, 앱은 0 이하나 3km 를 넘는
    도보 반경이 오면 코드 기본값으로 돌아갑니다.
