    ...
    if (status === 'invalid') {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 p-4">
                <div className="w-20 h-20 bg-rose-100 dark:bg-rose-900/30 rounded-3xl flex items-center justify-center text-rose-500 mb-6 font-semibold">
                    <span className="material-symbols-rounded text-4xl">link_off</span>
                </div>
                <h1 className="text-2xl font-bold mb-2">유효하지 않은 초대 링크입니다.</h1>
                <p className="text-slate-500 mb-8 font-medium">링크가 만료되었거나 올바르지 않습니다.</p>
                <button onClick={() => router.push('/')} className="px-8 py-4 bg-slate-900 dark:bg-white dark:text-slate-900 text-white font-semibold rounded-2xl">
                    홈으로 가기
                </button>
            </div>
        );
        ...
        <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 p-4">
            ...
            >
                <div className="w-20 h-20 bg-primary/10 rounded-3xl flex items-center justify-center text-primary mx-auto mb-8">
                    <span className="material-symbols-rounded text-4xl font-bold">rocket_launch</span>
                </div>

                <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-4 italic">Invitation</h1>
                <p className="text-slate-500 font-bold mb-8">
                    <span className="text-slate-900 dark:text-white font-bold block text-xl mb-1">"{trip?.title}"</span>
                ...
                <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-3xl mb-8 flex items-center justify-center gap-4">
                    ...
                    <div className="text-left">
                        <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Host</p>
                        <p className="text-sm font-semibold text-slate-900 dark:text-white">여행 마스터</p>
                    </div>
                </div>
                ...
                <button
                    ...
                    className="w-full py-5 bg-primary text-white font-semibold rounded-3xl shadow-xl shadow-primary/30 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-50"
                >
                    {status === 'joining' ? (
                        <div className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin" />
                    ) : (
                        <>
                            <span className="material-symbols-rounded font-semibold">person_add</span>
                            여행 참여하기
                        </>
                    )}
                </button>
