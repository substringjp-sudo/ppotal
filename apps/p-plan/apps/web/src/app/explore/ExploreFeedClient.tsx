...
function SpotFeedCard({ feed, liked, onLike }: { feed: FeedSpot; liked: boolean; onLike: () => void }) {
    ...
    return (
        <motion.article
            ...
            className="relative rounded-[2rem] overflow-hidden border border-slate-200 dark:border-white/10 bg-slate-900 shadow-lg"
        >
            {/* 커버 */}
            <Link href={`/spot/${feed.travelogId}/${spot.id}`} className="block relative aspect-[4/5]">
                ...
                {feed.theme && (
                    <span className="absolute top-4 left-4 rounded-full bg-black/50 backdrop-blur-md px-3 py-1 text-[10px] font-black uppercase tracking-widest text-white/90">
                        {feed.theme}
                    </span>
                )}
                {typeof spot.rating === 'number' && spot.rating > 0 && (
                    <span className="absolute top-4 right-4 flex items-center gap-0.5 rounded-full bg-black/50 backdrop-blur-md px-2.5 py-1">
                        ...
                        <span className="text-[11px] font-black text-white tabular-nums">{spot.rating}</span>
                    </span>
                )}
                {/* 하단 텍스트 오버레이 */}
                <div className="absolute inset-x-0 bottom-0 p-5">
                    <h2 className="text-2xl font-black text-white tracking-tight leading-tight">{spot.name}</h2>
                    {sub && <p className="mt-1 text-[11px] font-bold uppercase tracking-widest text-white/60">{sub}</p>}
                    {spot.impression && (
