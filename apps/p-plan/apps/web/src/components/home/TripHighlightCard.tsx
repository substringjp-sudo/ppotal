    ...
    <motion.section variants={itemVariants} className="relative group overflow-hidden bg-slate-900 rounded-[20px] p-4 lg:p-6 text-white min-h-[200px] lg:min-h-[240px] h-full flex flex-col justify-between shadow-xl">
      {targetTrip ? (
              ...
              <div className="relative z-10 flex flex-col 2xl:flex-row justify-between items-start gap-5 h-full">
                  <div className="flex-1 flex flex-col justify-between h-full">
                      <div>
                          <div className="flex items-center gap-2 mb-2">
                              <span className={cn(
                                  "px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-[0.2em] shadow-lg backdrop-blur-xl",
                              ...
                              {ongoingTrip && ongoingTrip.dates && (
                                  <span className="text-xs font-semibold text-white/70 uppercase tracking-widest bg-white/10 px-2 py-1 rounded-lg">Day {differenceInDays(startOfDay(new Date()), parseISO(ongoingTrip.dates.startDate)) + 1} of {differenceInDays(parseISO(ongoingTrip.dates.endDate), parseISO(ongoingTrip.dates.startDate)) + 1}</span>
                              )}
                          </div>
                          <h2 className="text-2xl md:text-4xl font-bold mb-2 leading-tight tracking-tight drop-shadow-2xl">
                              <span className="font-display tracking-tight">{targetTrip.title}</span>
                          </h2>
                          <div className="flex flex-wrap items-center gap-4 text-white font-bold uppercase tracking-widest text-xs">
                              ...
                              {!ongoingTrip && targetTrip.dates?.startDate && (
                                  <div className="flex items-center gap-2.5 px-3 py-1.5 bg-white/10 rounded-xl backdrop-blur-md border border-white/5 group-hover:bg-primary group-hover:text-white transition-all text-primary font-semibold shadow-lg shadow-black/10">
                      ...
                      <div className="mt-4 flex flex-wrap items-center gap-3">
                          <div className="flex items-center gap-3">
                              <div className="flex -space-x-2">
                                  <div className="w-10 h-10 rounded-xl border-2 border-slate-950 overflow-hidden bg-primary/20 ring-2 ring-white/10 transition-transform hover:scale-110 cursor-pointer relative group/avatar flex items-center justify-center">
                                      {user?.photoURL ? (
                                          ...
                                          <span className="text-xs font-semibold text-primary">{user?.displayName?.[0] || 'U'}</span>
                                      )}
                                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/avatar:opacity-100 transition-opacity flex items-center justify-center">
                                          <span className="text-xs font-semibold uppercase text-white">Me</span>
                                      </div>
                                  </div>
                                  {(targetTrip.participantCount || 0) > 0 && (
                                      <div 
                                          title="파트너와 공유 중인 여행입니다"
                                          className="w-10 h-10 rounded-xl border-2 border-slate-950 bg-slate-800/80 backdrop-blur-md flex items-center justify-center text-xs font-semibold ring-2 ring-white/10 text-primary hover:text-white transition-colors cursor-pointer"
                                      >
                              ...
                              <div className="flex flex-col">
                                  <span className="text-xs font-semibold text-white/40 uppercase tracking-widest leading-none mb-1">공유 상태</span>
                                  <span className={cn(
                          ...
                          <div className="flex gap-8">
                              <div className="flex flex-col">
                                  <span className="text-xs font-semibold text-white/40 uppercase tracking-widest leading-none mb-1">항공편</span>
                                  <span className="text-xl font-bold leading-none tabular-nums not-italic">{targetTrip.flightCount || 0}</span>
                              </div>
                              <div className="flex flex-col">
                                  <span className="text-xs font-semibold text-white/40 uppercase tracking-widest leading-none mb-1">숙소</span>
                                  <span className="text-xl font-bold leading-none tabular-nums not-italic">{targetTrip.accommodationCount || 0}</span>
                              </div>
                          </div>
                      </div>
                  </div>
                  ...
                  <div className="w-full 2xl:w-[260px] shrink-0 bg-white/5 backdrop-blur-2xl rounded-2xl border border-white/10 p-3.5 flex flex-col justify-between gap-3 hover:bg-white/10 transition-all duration-500">
                      <div className="flex justify-between items-start">
                          <div>
                              <p className="text-xs font-semibold text-primary uppercase tracking-[0.2em] mb-1">현재 상태</p>
                              <h3 className="text-xl font-bold">{ongoingTrip ? '오늘 일정' : '다음 일정'}</h3>
                          </div>
                          <div className="text-right">
                              {targetTrip.dates?.startDate && (
                                  <>
                                      <div className="text-2xl font-bold leading-tight tabular-nums not-italic">
                                          {ongoingTrip 
                                              ? `${differenceInDays(startOfDay(new Date()), parseISO(targetTrip.dates.startDate)) + 1}일차`
                                              : `D-${differenceInDays(parseISO(targetTrip.dates.startDate), startOfDay(new Date()))}`
                                          }
                                      </div>
                                      <p className="text-xs font-bold text-white/40 mt-1 uppercase">
                      ...
                      <div className="space-y-4">
                          {nextMilestone ? (
                              <div className="flex items-center gap-4 group/subItem cursor-pointer">
                                  ...
                                  <div>
                                      <p className="text-xs font-semibold group-hover/subItem:text-primary transition-colors">{nextMilestone.title}</p>
                                      <p className="text-xs font-bold text-white/30 uppercase tracking-tighter">{nextMilestone.time} · 예정된 일정</p>
                                  </div>
                              </div>
                          ) : (
                              <button
                              ...
                              >
                                 ...
                                 <div className="text-left">
                                     <p className="text-xs font-semibold">일정 추가하기</p>
                                     <p className="text-xs font-bold text-white/30 uppercase tracking-tighter">아직 등록된 일정이 없어요</p>
                                 </div>
                              </button>
                          )}
                      </div>
                      ...
                      >
                         <span className="text-xs font-semibold uppercase tracking-[0.2em] text-white/40 mb-0.5">스크롤하여 더 보기</span>
          ...
          <div className="relative z-10 text-center py-20 flex flex-col items-center flex-1 justify-center">
              ...
              >
                  <span className="material-symbols-rounded text-5xl text-primary font-bold">explore</span>
              </motion.div>
              <h2 className="text-4xl font-bold mb-4">어디로 떠나고 싶으신가요?</h2>
              ...
              <button
                ...
                className="px-10 py-5 bg-primary text-white font-semibold rounded-2xl shadow-2xl shadow-primary/30 hover:scale-105 transition-all"
              >
                  첫 번째 여행 계획하기
              </button>
          </div>
