  ...
  return (
    <motion.div variants={itemVariants} className="md:col-span-1 p-6 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden relative group">
        <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-xs font-semibold text-slate-400 tracking-widest uppercase">Live</span>
        </div>
        <div className="space-y-4">
            {activities.length > 0 ? (
                activities.slice(0, 3).map((act) => (
                    <div key={act.id} className="flex gap-3 items-center group/item hover:bg-slate-50 dark:hover:bg-slate-800/50 p-2 rounded-xl transition-all">
                        ...
                        <div className="min-w-0">
                            <p className="text-xs font-bold text-slate-700 dark:text-slate-300 truncate leading-snug">{act.message}</p>
                            <p className="text-xs font-semibold text-slate-400 uppercase tracking-tighter mt-0.5">{format(new Date(act.createdAt), 'MMM dd, HH:mm')}</p>
                        </div>
                    </div>
                ))
            ) : (
                <div className="p-10 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl text-center bg-slate-50/30 dark:bg-slate-900/10">
                  ...
                  <p className="text-xs text-slate-400 uppercase tracking-widest font-semibold">여행 계획을 수정하거나 장소를 추가해보세요!</p>
                </div>
            )}
        </div>
        <button className="w-full mt-4 py-2 text-xs font-semibold text-slate-500 hover:text-primary transition-colors border-t border-slate-200/60 dark:border-slate-800 pt-3 uppercase tracking-widest">
            모든 활동 보기
        </button>
    </motion.div>
  );
