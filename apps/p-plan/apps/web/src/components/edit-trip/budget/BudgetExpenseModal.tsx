            ...
            <div className="relative w-full max-w-xl bg-white dark:bg-slate-900 rounded-[40px] shadow-2xl overflow-hidden flex flex-col border border-slate-200 dark:border-slate-800">
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-6">
                    <div className="flex flex-col gap-1">
                        <label className="text-xs font-semibold text-slate-400 uppercase ml-1">지출 항목 명칭</label>
                        <input 
                            type="text"
                            value={title}
                        ...
                        />
                        {initialData?.isAuto && (
                            <span className="text-xs font-bold text-slate-400 ml-1">※ 자동 수집된 항목의 명칭은 출처에서 수정할 수 있습니다.</span>
                        )}
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                        <div className="flex flex-col gap-1">
                            <label className="text-xs font-semibold text-slate-400 uppercase ml-1">금액 및 통화</label>
                            <div className="flex gap-2">
                                <select 
                                    value={currency}
                                    onChange={(e) => setCurrency(e.target.value)}
                                    className="w-24 min-w-[96px] bg-slate-50 dark:bg-slate-800 rounded-xl px-2 py-3 text-xs font-semibold outline-none border-none cursor-pointer"
                                >
                            ...
                            {currency !== (baseCurrency || 'KRW') && (
                                <div className="mt-2 p-3 bg-primary/5 dark:bg-primary/10 rounded-2xl border border-primary/20">
                                    <div className="flex justify-between items-center">
                                        <span className="text-xs font-semibold text-primary/60 uppercase">실시간 환산 미리보기</span>
                                        <span className="text-xs font-bold text-slate-400">1 {currency} = {getRate(currency)} {baseCurrency}</span>
                                    </div>
                                    <div className="flex items-end gap-1 mt-1">
                                        <span className="text-lg font-semibold text-primary">{baseCurrency === 'KRW' ? '₩' : currencySymbol} {formatAmount(Math.round(Number(amount) * getRate(currency)))}</span>
                                        <span className="text-xs font-bold text-primary/40 mb-1">상당</span>
                                    </div>
                                </div>
                            )}
                        </div>
                        <div className="flex flex-col gap-1">
                            <label className="text-xs font-semibold text-slate-400 uppercase ml-1">카테고리</label>
                            <IconDropdown
                                value={category}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="flex flex-col gap-1">
                            <label className="text-xs font-semibold text-slate-400 uppercase ml-1">결제 수단</label>
                            <IconDropdown
                                value={paymentMethod}
                        ...
                        <div className="flex flex-col gap-1">
                            <label className="text-xs font-semibold text-slate-400 uppercase ml-1">결제 상태</label>
                            <IconDropdown
                                value={paymentStatus}
                    ...
                    <div className="flex flex-col gap-1">
                        <label className="text-xs font-semibold text-slate-400 uppercase ml-1">결제일 (선택)</label>
                        <input 
                            type="date"
                            value={paymentDate}
                    ...
                    {!isSoloTrip && (
                        <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-800">
                            <div className="flex flex-col gap-1">
                                <label className="text-xs font-semibold text-slate-400 uppercase ml-1">실제 결제자</label>
                            ...
                            <div className="flex flex-col gap-1">
                                <div className="flex justify-between items-center ml-1">
                                    <label className="text-xs font-semibold text-slate-400 uppercase">분담 멤버</label>
                                    <button 
                                        type="button"
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {tripParticipants.map(p => (
                                        <button 
                                            ...
                                            className={cn(
                                                "px-4 py-2 rounded-full text-xs font-semibold transition-all flex items-center gap-1.5",
                                ...
                                {splitWithIds.length === 0 && (
                                    <p className="text-xs text-rose-500 font-bold mt-1">* 최소 한 명 이상의 분담자를 선택해야 합니다.</p>
                                )}
                            </div>
                        </div>
                    )}
                    ...
                    <div className="flex flex-col gap-1">
                        <label className="text-xs font-semibold text-slate-400 uppercase ml-1">지출 상태</label>
                        <div className="flex gap-2">
                            {(['confirmed', 'planned'] as const).map(s => (
                                <button
                                    ...
                                    onClick={() => setStatus(s)}
                                    className={cn(
                                        "flex-1 py-3 rounded-xl text-xs font-semibold transition-all border",
                                        status === s 
                    ...
                    <button 
                        ...
                        className="w-full py-4 bg-primary text-white rounded-2xl font-semibold shadow-xl shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:scale-100"
                    >
