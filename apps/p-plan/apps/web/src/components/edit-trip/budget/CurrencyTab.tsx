    ...
    return (
        <div className="space-y-8">
            <div className="max-w-2xl">
                <h4 className="text-xs font-semibold text-slate-400 gap-2 flex items-center mb-4 uppercase tracking-[0.2em] ml-2">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {CURRENCY_OPTIONS.slice(0, 4).map(opt => (
                        ...
                        >
                            ...
                            <span className="text-xs font-semibold uppercase tracking-widest">{opt.value}</span>
                        </button>
                    ))}
                </div>
            </div>
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {(trip.budget?.activeCurrencies || []).map(curr => {
                        ...
                        return (
                            <div key={curr.code} className="bg-white dark:bg-slate-900 p-6 rounded-[20px] border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col gap-4 relative group">
                                ...
                                <div className="grid grid-cols-2 gap-4 pt-2">
                                    <div className="space-y-1">
                                        <label className="text-xs font-semibold text-slate-400 uppercase ml-1">활성 환율 (Rate)</label>
                                        <div className="relative">
                                            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400">1 {curr.code} =</span>
                                            <input 
                                                type="number"
                                                value={curr.rate}
                                                onChange={(e) => updateActiveCurrency(curr.code, { rate: Number(e.target.value) })}
                                                className="w-full pl-[52px] pr-8 py-2.5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-xs font-semibold focus:ring-2 focus:ring-primary/20 outline-none"
                                            />
                                            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400">{trip.budget?.baseCurrency || 'KRW'}</span>
                                        </div>
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-xs font-semibold text-slate-400 uppercase ml-1">통화 심볼 (Symbol)</label>
                                        <input 
                                            type="text"
                                            value={curr.symbol}
                                            onChange={(e) => updateActiveCurrency(curr.code, { symbol: e.target.value })}
                                            className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-xs font-semibold focus:ring-2 focus:ring-primary/20 outline-none"
                                        />
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="min-h-[160px] bg-slate-50/50 dark:bg-slate-800/10 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-[20px] p-6 flex flex-col justify-center">
                        <label className="text-xs font-semibold text-slate-400 uppercase tracking-widest ml-2 mb-4 block">통화 추가하기</label>
                        <div className="flex flex-wrap gap-2">
                            {availableCurrencyOptions.map(opt => (
                                <button
                                    ...
                                    className="px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-semibold uppercase tracking-widest hover:border-primary/40 hover:text-primary transition-all flex items-center gap-2 group"
                                >
                                    {opt.tag && (
                                        <span className={cn("px-1.5 py-0.5 rounded text-xs font-semibold uppercase", opt.tagColor)}>
                                            {opt.tag}
                                        </span>
                                    )}
                                    {opt.value}
