        ...
        <div className="space-y-8 pb-12">
            ...
            {transfers.length > 0 ? (
                <div className="space-y-4">
                    <h3 className="text-sm font-semibold flex items-center gap-2 ml-2">
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    <div className="flex flex-col items-center">
                                        <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-xs font-semibold">{getParticipantName(t.from).charAt(0)}</div>
                                        <span className="text-xs font-semibold mt-1">{getParticipantName(t.from)}</span>
                                    </div>
                                    <div className="flex flex-col items-center px-4">
                                        <span className="material-symbols-rounded text-emerald-500 text-xl">arrow_forward</span>
                                        <span className="text-xs font-semibold text-emerald-500/60 uppercase">송금</span>
                                    </div>
                                    <div className="flex flex-col items-center">
                                        <div className="w-8 h-8 rounded-full bg-primary text-white flex items-center justify-center text-xs font-semibold">{getParticipantName(t.to).charAt(0)}</div>
                                        <span className="text-xs font-semibold mt-1">{getParticipantName(t.to)}</span>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <div className="text-lg font-semibold text-emerald-600">{currencySymbol} {formatAmount(t.amount)}</div>
                                    <span className="text-xs font-bold text-slate-400">보내면 정산 완료!</span>
                                </div>
                            </motion.div>
                        ))}
                    </div>
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-slate-200 dark:border-slate-800">
                                <th className="px-6 py-4 text-xs font-semibold text-slate-400 uppercase">멤버</th>
                                <th className="px-6 py-4 text-xs font-semibold text-slate-400 uppercase text-right">총 결제액</th>
                                <th className="px-6 py-4 text-xs font-semibold text-slate-400 uppercase text-right">내 분담액</th>
                                <th className="px-6 py-4 text-xs font-semibold text-slate-400 uppercase text-right">정산 결과</th>
                            </tr>
                        </thead>
                            ...
                            {summary.map((s) => (
                                <tr key={s.participantId} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-6 h-6 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-xs font-semibold">{getParticipantName(s.participantId).charAt(0)}</div>
                                            <span className="text-xs font-semibold">{getParticipantName(s.participantId)}</span>
                                        </div>
                                    </td>
