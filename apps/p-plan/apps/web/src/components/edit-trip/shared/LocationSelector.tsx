    ...
    if (isGuest) {
        return (
            <div className={cn("space-y-2", className)}>
                <label className="text-xs font-semibold text-slate-700 dark:text-slate-300 ml-1 block">
                    {label || '장소이름'}
                </label>
                <input
                    type="text"
                    ...
                    >
                        <div className={cn(
                                ...
                                <div className={cn("flex flex-wrap items-center", compact ? "gap-1.5" : "gap-2")}>
                                    {regionPath?.country ? (
                                        <div className={cn(
                                            ...
                                            {regionPath.country}
                                            {selectedPlace.countryId && <span className="text-xs opacity-30 font-mono ml-1">{selectedPlace.countryId}</span>}
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2 text-xs font-bold text-primary animate-pulse pl-1 italic">
                                    ...
                                    {regionPath?.prefecture && (
                                        <>
                                            ...
                                            <div className={cn(
                                                ...
                                                {regionPath.prefecture}
                                                {selectedPlace.prefectureId && <span className="text-xs opacity-30 font-mono ml-1">{selectedPlace.prefectureId}</span>}
                                            </div>
                                        </>
                                    )}
                                    ...
                                    {regionPath?.city && (
                                        <>
                                            ...
                                            <div className={cn(
                                                ...
                                                {regionPath.city}
                                                {selectedPlace.cityId && <span className="text-xs opacity-30 font-mono ml-1">{selectedPlace.cityId}</span>}
                                            </div>
                                        </>
                                    )}
                                </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {selectedPlace.countryId && (
                                    <span className={cn(
                                        "px-2 py-0.5 bg-white dark:bg-slate-900 rounded-lg text-xs font-bold text-slate-500 border border-slate-200 dark:border-slate-800 flex items-center gap-1.5 shadow-sm transition-all",
                                        isResolvingRegion && !selectedPlace.countryName && "animate-pulse"
                                    )}>
                                        <span className="text-xs opacity-40 font-mono">CID</span>
                                ...
                                {selectedPlace.prefectureId && (
                                    <span className={cn(
                                        ...
                                        <span className="text-xs opacity-40 font-mono">PID</span>
                                ...
                                {selectedPlace.cityId && (
                                    <span className={cn(
                                        ...
                                        <span className="text-xs opacity-40 font-mono">TID</span>
                            ...
                            <div className="space-y-1">
                                ...
                                {selectedPlace.url && (
                                    <a 
                                        ...
                                        className={cn("inline-flex items-center gap-1 font-semibold text-primary hover:underline", compact ? "text-xs ml-5" : "text-xs ml-5")}
                                    >
                                        구글 맵에서 보기
                    ...
                    <div className="flex flex-wrap gap-2">
                        {nearbyRecommendations.map((place) => (
                            <button
                                ...
                                className="px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-semibold hover:border-primary/50 hover:bg-primary/5 hover:text-primary transition-all flex items-center gap-1.5"
                            >
                        ...
                        {(initialBias || (selectedPlace?.lat && selectedPlace?.lng)) && (
                            <button
                                ...
                                className="px-3 py-2 bg-slate-100 dark:bg-slate-800 border-dashed border-slate-300 dark:border-slate-700 rounded-xl text-xs font-semibold text-slate-500 hover:text-primary hover:border-primary transition-all flex items-center gap-1.5"
                            >
