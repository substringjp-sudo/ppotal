...
exports.parseTripFromJSON = parseTripFromJSON;
function parseTripFromJSON(jsonData) {
    try {
        ...
        const resetListIds = (list) => {
            if (!Array.isArray(list))
                return [];
            return list.map(item => ({ ...item, id: crypto.randomUUID() }));
        };
        if (newTrip.checklist)
            newTrip.checklist = resetListIds(newTrip.checklist);
        if (newTrip.bucketList)
            newTrip.bucketList = resetListIds(newTrip.bucketList);
        if (newTrip.flights)
            newTrip.flights = resetListIds(newTrip.flights);
        if (newTrip.accommodation)
            newTrip.accommodation = resetListIds(newTrip.accommodation);
        if (newTrip.driving)
            newTrip.driving = resetListIds(newTrip.driving);
        if (newTrip.publicTransport)
            newTrip.publicTransport = resetListIds(newTrip.publicTransport);
        if (newTrip.prepTimeline)
            newTrip.prepTimeline = resetListIds(newTrip.prepTimeline);
        if (newTrip.reservations)
            newTrip.reservations = resetListIds(newTrip.reservations);
        if (newTrip.dailyTimeline && Array.isArray(newTrip.dailyTimeline)) {
            newTrip.dailyTimeline = newTrip.dailyTimeline.map((day) => ({
                ...day,
