...
exports.validateFlightCompleteness = validateFlightCompleteness;
exports.validateFlightTimeRange = validateFlightTimeRange;
exports.validateFlightSpeed = validateFlightSpeed;
exports.validateRentalCarPeriod = validateRentalCarPeriod;
exports.validatePublicTransportFeasibility = validatePublicTransportFeasibility;
exports.validateDrivingFeasibility = validateDrivingFeasibility;
exports.validateFlightLayovers = validateFlightLayovers;
exports.validatePublicTransportConflicts = validatePublicTransportConflicts;
exports.validateDrivingConflicts = validateDrivingConflicts;
const airports_1 = require("../airports");
const geo_utils_1 = require("../geo-utils");
const utils_1 = require("../utils");
function validateFlightCompleteness(trip, warnings, style) {
    if (!trip.transportSettings?.useFlight)
        return;
    ...
    const baseSeverity = style?.planning === 'flexible' ? 'info' : 'warning';
...
function validateFlightTimeRange(trip, warnings) {
    (trip.flights || []).forEach(f => {
        const depMinutes = (0, utils_1.timeToMinutes)(f.departureTime);
        const arrMinutes = (0, utils_1.timeToMinutes)(f.arrivalTime);
        const isEarlyMorning = (m) => m !== null && m >= 0 && m <= 4 * 60;
...
function validateFlightSpeed(trip, warnings) {
    (trip.flights || []).forEach(f => {
        ...
        if (!aLat && f.arrivalLocation) {
            const a = airports_1.AIRPORTS.find((ap) => ap.code === f.arrivalLocation);
            if (a) {
                aLat = a.lat;
                aLng = a.lng;
            }
        ...
        if (dLat && dLng && aLat && aLng && f.flightDurationMinutes) {
            const dist = (0, geo_utils_1.calculateDistance)(dLat, dLng, aLat, aLng);
...
function validateRentalCarPeriod(trip, warnings) {
...
function validatePublicTransportFeasibility(trip, warnings) {
    (trip.publicTransport || []).forEach(pt => {
        if (pt.departureLat && pt.departureLng && pt.arrivalLat && pt.arrivalLng && pt.departureTime && pt.arrivalTime) {
            const dist = (0, geo_utils_1.calculateDistance)(pt.departureLat, pt.departureLng, pt.arrivalLat, pt.arrivalLng);
            const start = (0, utils_1.timeToMinutes)(pt.departureTime);
            let end = (0, utils_1.timeToMinutes)(pt.arrivalTime);
            if (pt.isNextDayArrival)
                end += 24 * 60;
...
function validateDrivingFeasibility(trip, warnings) {
    (trip.driving || []).forEach(d => {
        if (d.pickupLat && d.pickupLng && d.returnLat && d.returnLng && d.pickupTime && d.returnTime && !d.isReturnSameAsPickup) {
            const dist = (0, geo_utils_1.calculateDistance)(d.pickupLat, d.pickupLng, d.returnLat, d.returnLng);
            const start = (0, utils_1.timeToMinutes)(d.pickupTime);
            const end = (0, utils_1.timeToMinutes)(d.returnTime);
            const durationHrs = (end - start) / 60;
            if (durationHrs <= 0 && dist > 1) {
                warnings.push({
...
function validateFlightLayovers(trip, warnings) {
    (trip.flights || []).forEach(f => {
 ...
 */
function validatePublicTransportConflicts(trip, warnings) {
    const transports = trip.publicTransport || [];
    if (transports.length === 0)
        return;
    transports.forEach(pt => {
        if (!pt.date || !pt.departureTime || !pt.arrivalTime)
            return;
        const ptStart = (0, utils_1.timeToMinutes)(pt.departureTime);
        let ptEnd = (0, utils_1.timeToMinutes)(pt.arrivalTime);
        if (pt.isNextDayArrival)
            ptEnd += 24 * 60;
        // 1. 항공편과 겹침 확인
        trip.flights?.forEach(flight => {
            if (flight.date !== pt.date || !flight.departureTime || !flight.arrivalTime)
                return;
            const fStart = (0, utils_1.timeToMinutes)(flight.departureTime);
            const fEnd = (0, utils_1.timeToMinutes)(flight.arrivalTime);
        ...
        transports.forEach(other => {
            if (pt.id === other.id || other.date !== pt.date || !other.departureTime || !other.arrivalTime)
                return;
            const oStart = (0, utils_1.timeToMinutes)(other.departureTime);
            let oEnd = (0, utils_1.timeToMinutes)(other.arrivalTime);
            if (other.isNextDayArrival)
                oEnd += 24 * 60;
        ...
        day?.events?.forEach(event => {
            if (!event.startTime || !event.endTime || !event.isFixedTime)
                return;
            const eStart = (0, utils_1.timeToMinutes)(event.startTime);
            const eEnd = (0, utils_1.timeToMinutes)(event.endTime);
 ...
 */
function validateDrivingConflicts(trip, warnings) {
    const drivings = trip.driving || [];
    if (drivings.length === 0)
        return;
    drivings.forEach(d => {
        if (!d.date || !d.pickupTime || !d.returnTime)
            return;
        const dStart = (0, utils_1.timeToMinutes)(d.pickupTime);
        const dEnd = (0, utils_1.timeToMinutes)(d.returnTime);
        // 항공편과 겹침 확인
        trip.flights?.forEach(flight => {
            if (flight.date !== d.date || !flight.departureTime || !flight.arrivalTime)
                return;
            const fStart = (0, utils_1.timeToMinutes)(flight.departureTime);
            const fEnd = (0, utils_1.timeToMinutes)(flight.arrivalTime);
