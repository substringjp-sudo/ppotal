...
exports.validateAirportDistance = validateAirportDistance;
exports.validateAccommodationRegion = validateAccommodationRegion;
exports.validateEventLocations = validateEventLocations;
exports.validateLocationClusters = validateLocationClusters;
const airports_1 = require("../airports");
const geo_utils_1 = require("../geo-utils");
function validateAirportDistance(trip, warnings) {
    if (!trip.locations || !trip.locations.center)
        return;
    const center = trip.locations.center;
    // 절대적으로 가장 가까운 공항 찾기
    let minDistance = Infinity;
    let closestAirportName = '';
    airports_1.AIRPORTS.forEach((a) => {
        if (a.lat && a.lng) {
            const d = (0, geo_utils_1.calculateDistance)(a.lat, a.lng, center.lat, center.lng);
            if (d < minDistance) {
                minDistance = d;
    ...
    (trip.flights || []).forEach(flight => {
        ...
        if (!lat && flight.arrivalLocation) {
            const a = airports_1.AIRPORTS.find((ap) => ap.code === flight.arrivalLocation);
            if (a) {
                lat = a.lat;
                lng = a.lng;
                locName = a.nameKo;
        ...
        if (flight.type === 'outbound' && lat && lng) {
            const dist = (0, geo_utils_1.calculateDistance)(lat, lng, center.lat, center.lng);
...
function validateAccommodationRegion(trip, warnings, geometries) {
    (trip.accommodation || []).forEach(acc => {
        // 1. 기하학적 경계 데이터가 있는 경우 우선 적용
        if (geometries && acc.lat && acc.lng) {
            const boundaryCheck = (0, geo_utils_1.checkPointInRegions)(acc.lat, acc.lng, trip.locations.regions || [], geometries);
        ...
        if (trip.locations?.center && acc.lat && acc.lng) {
            const dist = (0, geo_utils_1.calculateDistance)(acc.lat, acc.lng, trip.locations.center.lat, trip.locations.center.lng);
        ...
        if (acc.countryId || acc.prefectureId || acc.cityId) {
            const isInRegion = trip.locations.regions?.some(r => r.id === acc.countryId ||
...
function validateEventLocations(trip, warnings, geometries) {
    if (!trip.locations?.regions || trip.locations.regions.length === 0)
        return;
    (trip.dailyTimeline || []).forEach((day, dayIdx) => {
        (day.events || []).forEach(event => {
            // 1. 기하학적 경계 데이터가 있는 경우 우선 적용
            if (geometries && event.location?.lat && event.location?.lng) {
                const boundaryCheck = (0, geo_utils_1.checkPointInRegions)(event.location.lat, event.location.lng, trip.locations.regions || [], geometries);
                if (!boundaryCheck.isInside) {
                    const distanceMsg = boundaryCheck.minDistance > 0
                        ? ` (경계선으로부터 약 ${Math.round(boundaryCheck.minDistance)}km 떨어짐)`
                        : '';
            ...
            if (event.location?.countryId || event.location?.prefectureId || event.location?.cityId) {
                const isInRegion = trip.locations.regions?.some(r => r.id === event.location?.countryId ||
...
function validateLocationClusters(trip, warnings) {
    (trip.dailyTimeline || []).forEach((day, dayIdx) => {
        ...
        locations.forEach(loc => {
            const d = (0, geo_utils_1.calculateDistance)(avgLat, avgLng, loc.lat, loc.lng);
