...
exports.validateChecklistProgress = validateChecklistProgress;
exports.validatePrepTaskProgress = validatePrepTaskProgress;
function validateChecklistProgress(trip, warnings, style) {
...
function validatePrepTaskProgress(trip, warnings, style) {
    if (!trip.prepTimeline || trip.prepTimeline.length === 0)
        return;
    ...
    if (activeTasks.length > 0) {
        ...
        let severity = missedCount > 0 ? 'warning' : 'info';
        if (style?.meticulousness === 'forgetful') {
            severity = 'warning';
        }
        warnings.push({
