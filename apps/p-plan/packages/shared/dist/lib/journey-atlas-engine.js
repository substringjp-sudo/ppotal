...
exports.buildJourneyAtlas = buildJourneyAtlas;
exports.buildJourneyAtlasFromTravelogs = buildJourneyAtlasFromTravelogs;
exports.nodesToGeoJSON = nodesToGeoJSON;
exports.edgesToGeoJSON = edgesToGeoJSON;
 ...
 */
function buildJourneyAtlas(trips) {
    return buildFromTrips(trips);
}
/**
 * Travelog 배열을 JourneyAtlasData로 변환합니다.
 */
function buildJourneyAtlasFromTravelogs(travelogs) {
    const nodes = [];
    const edges = [];
    const tripMeta = [];
    // 시간순 정렬
 ...
 */
function mapTravelogEventType(event) {
    if (event.mainCategory === '숙박')
        return 'accommodation';
    if (event.mainCategory === '이동') {
        if (event.subCategory === '비행기')
            return 'flight-arrival'; // 단순화
    ...
    sorted.forEach((trip, tripIndex) => {
        ...
        if (trip.accommodation) {
            trip.accommodation.forEach((acc) => {
                if (!isValidCoord(acc.lat, acc.lng))
                    return;
                nodes.push({
...
function edgesToGeoJSON(edges) {
    return {
        ...
        features: edges.map(edge => {
            ...
            let coordinates;
            if (edge.type === 'flight') {
                coordinates = generateGreatCircleArc([edge.from.lng, edge.from.lat], [edge.to.lng, edge.to.lat], 50 // 50개의 중간점으로 부드러운 곡선 생성
                );
            }
            else {
                coordinates = [
 ...
 */
function generateGreatCircleArc(from, to, numPoints = 50) {
