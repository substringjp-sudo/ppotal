...
exports.getDistanceToSegment = getDistanceToSegment;
exports.getDistanceToPolygon = getDistanceToPolygon;
exports.getDistanceToMultiPolygon = getDistanceToMultiPolygon;
exports.checkLocationBoundary = checkLocationBoundary;
exports.checkPointInRegions = checkPointInRegions;
...
function getDistanceToSegment(p, a, b) {
    ...
    if (l2 === 0)
        return calculateDistance(py, px, ay, ax);
 ...
 */
function getDistanceToPolygon(p, polygon) {
    let minDistance = Infinity;
    for (const ring of polygon) {
        for (let i = 0; i < ring.length - 1; i++) {
            const dist = getDistanceToSegment(p, ring[i], ring[i + 1]);
            if (dist < minDistance)
                minDistance = dist;
        }
        // 마지막 점과 첫 점 연결
        const dist = getDistanceToSegment(p, ring[ring.length - 1], ring[0]);
        if (dist < minDistance)
            minDistance = dist;
    }
    return minDistance;
 ...
 */
function getDistanceToMultiPolygon(p, multiPolygon) {
    let minDistance = Infinity;
 ...
 */
function checkLocationBoundary(lat, lng, geometry) {
    const p = [lng, lat];
    let isInside = false;
    let distance = 0;
 ...
 */
function checkPointInRegions(lat, lng, regions, geometries) {
    let isInside = false;
    let minDistance = Infinity;
...
function simplifyGeometry(geometry, tolerance) {
    if (!geometry || typeof geometry !== 'object' || tolerance <= 0)
        return geometry;
    if (!('type' in geometry)) {
        return geometry;
    }
    if (geometry.type === 'Polygon') {
        const coordinates = geometry.coordinates.map(ring => simplifyPoints(ring, tolerance));
        return { ...geometry, coordinates };
    }
    else if (geometry.type === 'MultiPolygon') {
        const coordinates = geometry.coordinates.map(polygon => polygon.map(ring => simplifyPoints(ring, tolerance)));
        return { ...geometry, coordinates };
    }
    return geometry;
}
