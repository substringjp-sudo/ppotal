...
exports.DEFAULT_EXCHANGE_RATES = exports.CURRENCY_SYMBOLS = exports.WORLD_CURRENCIES = exports.COUNTRY_CURRENCY_MAP = void 0;
exports.getExchangeRate = getExchangeRate;
exports.convertCurrency = convertCurrency;
exports.inferCurrencyFromRegions = inferCurrencyFromRegions;
exports.getCountryCurrencies = getCountryCurrencies;
exports.getCurrencySymbol = getCurrencySymbol;
exports.calculateRecommendedExchangeDate = calculateRecommendedExchangeDate;
 ...
 */
function getExchangeRate(code, dynamicRates) {
    if (dynamicRates && dynamicRates[code]) {
        return dynamicRates[code];
    }
    return exports.DEFAULT_EXCHANGE_RATES[code] || 1;
}
/**
 * 두 통화 간의 환율을 계산합니다. (1 from = ? to)
 */
function convertCurrency(amount, fromCode, toCode, dynamicRates) {
    if (fromCode === toCode)
        return amount;
 ...
 */
function inferCurrencyFromRegions(regions, countryData) {
    ...
    for (const name of names) {
        ...
        for (const [country, code] of Object.entries(exports.COUNTRY_CURRENCY_MAP)) {
            if (lowerName.includes(country.toLowerCase()))
                return code;
        }
    }
    // 2. TripRegion 객체가 포함되어 있다면 ID 기반 매칭 (데이터가 있을 때만)
    const structuredRegions = regions.filter((r) => typeof r !== 'string');
 ...
 */
function getCountryCurrencies(countryNameOrId, countryData) {
    if (!countryNameOrId || !countryData)
        return ['USD'];
 ...
 */
function calculateRecommendedExchangeDate(startDateStr) {
    if (!startDateStr)
        return '날짜 미정';
