...
exports.FirestoreAdminAviationProvider = void 0;
/**
 * Firestore Admin SDK 기반 항공 데이터 프로바이더 (Shared)
 * Cloud Functions 등 서버 사이드 환경에서 사용됩니다.
 */
class FirestoreAdminAviationProvider {
    db;
    _isReady = false;
    constructor(db) {
        this.db = db;
    ...
    async searchAirlines(queryStr) {
        ...
        const codeSnapshot = await this.db.collection('airlines')
            ...
            .limit(10)
            .get();
        codeSnapshot.forEach(doc => results.set(doc.id, doc.data()));
        if (results.size < 10) {
            // 2. 한글 이름으로 검색
            const koSnapshot = await this.db.collection('airlines')
                ...
                .get();
            koSnapshot.forEach(doc => results.set(doc.id, doc.data()));
        }
        if (results.size < 10) {
            // 3. 영문 이름으로 검색
            const enSnapshot = await this.db.collection('airlines')
                ...
                .limit(10 - results.size)
                .get();
            enSnapshot.forEach(doc => results.set(doc.id, doc.data()));
        }
...
exports.FirestoreAdminAviationProvider = FirestoreAdminAviationProvider;
