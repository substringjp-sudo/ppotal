...
export function clusterPhotos(
    ...
    maxDistanceMeter: number = 50,
    maxTimeGapMs: number = 2 * 60 * 60 * 1000
): ClusteredLocation[] {
    ...
    let currentCluster: PhotoMetadata[] = [sortedPhotos[0]];
...
function finalizeCluster(photos: PhotoMetadata[]): ClusteredLocation {
