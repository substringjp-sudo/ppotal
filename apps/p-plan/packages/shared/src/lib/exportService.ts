...
function icsDT(date: string, time: string): string {
...
export function exportToICS(trip: Trip | TripDocument): void {
    const title = trip.titleSuggestion || trip.title || '여행';
    const uid = (id: string) => `${id}@pplaner.ai`;
    const lines: string[] = [];

    const L = (s: string) => lines.push(foldLine(s));
    ...
    (trip.flights ?? []).forEach((f) => {
        if (!f.date && !f.departureTime) return;
        ...
        if (depDate && depTime) {
            ...
            if (arrTime) L(`DTEND:${icsDT(depDate, arrTime)}`);
            else L(`DTEND:${icsDT(depDate, depTime)}`);
        } else if (depDate) {
            L(`DTSTART;VALUE=DATE:${icsDate(depDate)}`);
            L(`DTEND;VALUE=DATE:${icsDate(addDays(depDate, 1))}`);
        }
        L(`SUMMARY:${esc(summary)}`);
        if (f.departureLocation) L(`LOCATION:${esc(f.departureLocation)}`);
        const desc: string[] = [];
        if (flightNum) desc.push(`항공편: ${flightNum}`);
        if (f.cost != null) desc.push(`비용: ${f.cost.toLocaleString()} ${f.currency ?? ''}`);
        if (desc.length) L(`DESCRIPTION:${esc(desc.join('\\n'))}`);
        L('END:VEVENT');
    });
    ...
    (trip.accommodation ?? []).forEach((a) => {
        ...
        if (a.location) L(`LOCATION:${esc(a.location)}`);
        const desc: string[] = [];
        if (a.checkOutEndTime) desc.push(`체크아웃: ${a.endDate ?? ''} ${a.expectedCheckOutTime ?? a.checkOutEndTime ?? ''}`);
        if (a.price != null) desc.push(`요금: ${a.price.toLocaleString()} ${a.currency ?? ''}`);
        if (desc.length) L(`DESCRIPTION:${esc(desc.join('\\n'))}`);
        L('END:VEVENT');
    });
    ...
    (trip.dailyTimeline ?? []).forEach((day) => {
        (day.events ?? []).forEach((ev) => {
            ...
            if (ev.startTime) {
                ...
                if (ev.endTime) L(`DTEND:${icsDT(day.date, ev.endTime)}`);
                else L(`DTEND:${icsDT(day.date, ev.startTime)}`);
            } else {
            ...
            if (ev.location?.name) L(`LOCATION:${esc(ev.location.name + (ev.location.address ? `, ${ev.location.address}` : ''))}`);
            const desc: string[] = [];
            if (ev.memo) desc.push(ev.memo);
            if (ev.cost != null) desc.push(`비용: ${ev.cost.toLocaleString()} ${ev.currency ?? ''}`);
            if (desc.length) L(`DESCRIPTION:${esc(desc.join('\\n'))}`);
            L('END:VEVENT');
        });
    });
    ...
    (trip.reservations ?? []).forEach((r) => {
        ...
        if (r.location) L(`LOCATION:${esc(r.location)}`);
        if (r.memo) L(`DESCRIPTION:${esc(r.memo)}`);
        L('END:VEVENT');
    });
...
export function exportToJSON(trip: Trip | TripDocument): void {
    const title = trip.titleSuggestion || trip.title || '여행';
    const { _loadedSubCollections, ...exportData } = trip as TripDocument & { _loadedSubCollections?: unknown };
...
export function exportToCSV(trip: Trip | TripDocument): void {
    const title = trip.titleSuggestion || trip.title || '여행';
...
export function exportToPrint(trip: Trip | TripDocument): void {
    ...
    type ScheduleRow = { date: string; dow: string; time: string; type: string; title: string; location: string; status: string; memo: string };
