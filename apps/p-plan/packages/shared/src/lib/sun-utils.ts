 ...
 */
export function getSunPhases(date: string | Date, lat?: number, lng?: number): SunPhase {
    // Default values if location is missing
    const defaultPhase: SunPhase = {
        sunrise: '07:00',
        sunset: '19:00',
        sunriseMins: 7 * 60,
        sunsetMins: 19 * 60
    ...
    if (lat === undefined || lng === undefined) return defaultPhase;

    try {
        const d = typeof date === 'string' ? new Date(date) : date;
        if (isNaN(d.getTime())) return defaultPhase;
