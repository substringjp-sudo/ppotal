import { 
    Trip, TripSummary, Travelog, WishlistItem, 
    TravelStats, MasteryNode, RegionType 
} from '../types/index';

const ACHIEVEMENTS = [
    { id: 'first_step', title: '첫 번째 발자국', description: '첫 번째 여행 계획을 세웠습니다.', icon: '👣', maxXp: 100, category: 'milestones' as const },
    { id: 'world_traveler', title: '세계 여행자', description: '3개 이상의 국가를 방문했습니다.', icon: '🌍', maxXp: 500, category: 'destinations' as const },
    { id: 'record_master', title: '기록의 마스터', description: '10개 이상의 트래블로그를 작성했습니다.', icon: '✍️', maxXp: 300, category: 'records' as const },
    { id: 'planner', title: '철두철미 플래너', description: '일정 밀도가 매우 높은 여행을 계획했습니다.', icon: '📅', maxXp: 200, category: 'planning' as const },
];
...
export const calculateTravelStats = (
    ...
    regionNameMap?: { id: string, name: string }[],
    ...
    const visitedPrefectureIds = new Set<string>();
    const visitedCityIds = new Set<string>();
    const idToNameMap = new Map<string, string>();
    const mastery: Record<string, MasteryNode> = {};

    // 2. Helper functions
    const normalizeId = (id: string | number | undefined | null, type: RegionType): string | undefined => {
        if (id === null || id === undefined) return undefined;
        const sId = String(id).trim();
        if (!sId || sId === 'undefined' || sId === 'null') return undefined;
        
        // Ensure format is type_id
        if (sId.startsWith(`${type}_`)) return sId;
    ...
    const getMasteryNode = (type: RegionType, id: string, name: string): MasteryNode => ({
        id, 
        name: name || '알 수 없는 지역', 
        type, 
        xp: 0, 
    ...
    trips.forEach(trip => {
        try {
            ...
            totalDays += tripDays;
            if (isPlanned) totalPlannedDays += tripDays;
        
            // Safe flight/accommodation counts
            const currentFlights = Number((trip as any).flightCount || (trip as any).flights?.length || 0) || 0;
            const currentHotels = Number((trip as any).accommodationCount || (trip as any).accommodation?.length || 0) || 0;
            ...
            regions.forEach(r => {
                if (!r || !r.id) return;
                const normalizedIdVal = normalizeId(r.id, r.type as any);
                if (!normalizedIdVal) return;
                ...
                if (r.type === 'country' || r.countryId) {
                    ...
                    if (countryId) {
                        ...
                        if (!mastery[countryId]) {
                            mastery[countryId] = { ...getMasteryNode('country', countryId, r.type === 'country' ? r.name : (r as any).countryName), prefectures: {} };
                        }
                        ...
                        if (r.type === 'country') {
                            ...
                            const tripBudget = (trip as any).budget;
                ...
                if ((r.type === 'prefecture' || r.prefectureId) && r.countryId) {
                    ...
                    if (mastery[countryId]) {
                        ...
                        const cNode = mastery[countryId]!;
                        if (!cNode.prefectures) cNode.prefectures = {};
                        
                        if (!cNode.prefectures[prefId]) {
                            cNode.prefectures[prefId] = { ...getMasteryNode('prefecture', prefId, r.type === 'prefecture' ? r.name : (r as any).prefectureName), cities: {} };
                        }
                ...
                if (r.type === 'city' && r.countryId && r.prefectureId) {
                    ...
                    if (mastery[countryId]) {
                        ...
                        if (cNode.prefectures && cNode.prefectures[prefId]) {
                            ...
                            if (!prefNode.cities) prefNode.cities = {};
            ...
            if ('dailyTimeline' in trip && Array.isArray(trip.dailyTimeline)) {
                trip.dailyTimeline.forEach(day => {
                    ...
                    day.events.forEach(e => {
                        if (!e) return;
                        totalEvents++;
                        const mainCat = (e.mainCategory || '기타') as string;
                        if (mainCat === '식사' || mainCat === 'meal') foodCount++;
                        if (mainCat === '방문' || mainCat === '참여' || mainCat === 'sightseeing' || mainCat === 'activity') activityCount++;
                    });
                });
            }
        } catch (err) {
    ...
    (travelogs || []).forEach(log => {
        try {
            ...
            if (!Array.isArray(log.timeline)) return;
    ...
    const getFantasyClass = () => {
        if (persona.p > 80 && persona.t > 70) return "Grand Strategist (대마법사)";
        if (persona.a > 80 && persona.t > 60) return "Paladin Explorer (성기사)";
        if (persona.a > 70 && persona.p < 40) return "Shadow Assassin (암살자)";
        if (persona.th > 70 && persona.p < 50) return "Ethereal Bard (음유시인)";
        if (persona.t > 80) return "Dragon Slayer (용사)";
        return "Free Spirit Voyager (방랑자)";
    };

    // --- Helper for Wishlist Insights ---
    const calculateWishlistInsights = () => {
        if (!wishlistItems || wishlistItems.length === 0) return undefined;
        ...
        wishlistItems.forEach(item => {
            ...
            if (countryId) {
                if (!regionCounts[countryId]) regionCounts[countryId] = { name: countryName, count: 0 };
                regionCounts[countryId].count++;
            }
            const mainCat = item.mainCategory || 'other';
            const categoryLabel = (CATEGORY_MAP as any)[mainCat]?.label || '기타';
    ...
    const calculatePlanningStyle = (): TravelStats['planningStyle'] => {
