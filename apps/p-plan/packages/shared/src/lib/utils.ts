...
export function formatTripDuration(startDate?: string, endDate?: string, durationDays?: number) {
...
export function getAccommodationGaps(trip: { dates?: { startDate?: string, endDate?: string }, accommodation?: Array<{ startDate: string, endDate: string }> }): AccommodationGap[] {
    ...
    if (sortedAcc.length > 0) {
        ...
        if (firstAccStart > tripStart) {
            ...
            if (diff > 0) gaps.push({ start: trip.dates.startDate, end: sortedAcc[0].startDate, days: diff });
        }
    } else {
        // 숙소가 하나도 없는 경우 전체 기간이 공백
        const diff = getTripDurationDays(trip.dates.startDate, trip.dates.endDate) - 1;
        if (diff > 0) gaps.push({ start: trip.dates.startDate, end: trip.dates.endDate, days: diff });
        return gaps;
    }
    ...
    if (tripEnd > lastAccEnd) {
        ...
        if (diff > 0) gaps.push({ start: sortedAcc[sortedAcc.length - 1].endDate, end: trip.dates.endDate, days: diff });
    }
 ...
 */
export function isGoogleMapsReady(libraries: string[] = []): boolean {
    ...
    for (const lib of libraries) {
        if (lib === 'places' && !google.maps.places) return false;
        if (lib === 'geometry' && !google.maps.geometry) return false;
        if (lib === 'marker' && !google.maps.marker) return false;
        if (lib === 'visualization' && !google.maps.visualization) return false;
    }
