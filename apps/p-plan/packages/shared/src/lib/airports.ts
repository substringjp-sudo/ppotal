...
export function getRecommendedAirports(
    regions: TripRegion[], 
    ...
    const favoriteObjects = AIRPORTS.filter(a => favorites.includes(a.code));
    
    // 2. Residence matches (ID 기반 최우선)
    let residenceMatches: Airport[] = [];
    if (residence) {
        residenceMatches = AIRPORTS.filter(airport => {
            if (residence.countryId && airport.regionIds.countryId === residence.countryId) return true;
            if (residence.cityKo && airport.regionIds.cityName && (airport.regionIds.cityName.includes(residence.cityKo) || residence.cityKo.includes(airport.regionIds.cityName))) return true;
            if (residence.regionKo && airport.regionIds.prefectureName && (airport.regionIds.prefectureName.includes(residence.regionKo) || residence.regionKo.includes(airport.regionIds.prefectureName))) return true;
            if (residence.countryKo && airport.regionIds.countryName && (airport.regionIds.countryName.includes(residence.countryKo) || residence.countryKo.includes(airport.regionIds.countryName))) return true;
            return false;
        });

        residenceMatches.sort((a, b) => {
            const getScore = (air: Airport) => {
                if (residence.countryId && air.regionIds.countryId === residence.countryId) return 4;
                if (residence.cityKo && air.regionIds.cityName && air.regionIds.cityName.includes(residence.cityKo)) return 3;
                if (residence.regionKo && air.regionIds.prefectureName && air.regionIds.prefectureName.includes(residence.regionKo)) return 2;
                if (residence.countryKo && air.regionIds.countryName && air.regionIds.countryName.includes(residence.countryKo)) return 1;
                return 0;
            };
    ...
    let searchMatches: Airport[] = [];
    searchMatches = AIRPORTS.filter(airport => 
        !favorites.includes(airport.code) && 
        !residenceMatches.some(r => r.code === airport.code) &&
        regions.some(tr => {
            if (airport.regionIds.countryId && tr.countryId === airport.regionIds.countryId) return true;
            if (airport.regionIds.countryId && tr.type === 'country' && tr.id === airport.regionIds.countryId) return true;
...
export function isInternationalFlight(depCode: string, arrCode: string): boolean {
    if (!depCode || !arrCode) return false;
    const dep = AIRPORTS.find(a => a.code === depCode);
    const arr = AIRPORTS.find(a => a.code === arrCode);
