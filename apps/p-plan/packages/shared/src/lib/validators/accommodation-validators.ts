...
export function validateAccommodationOverlap(trip: Trip, warnings: TripWarning[], style?: TravelStyle) {
    if (!trip.accommodation || trip.accommodation.length < 2) return;
        ...
        for (let j = i + 1; j < accommodations.length; j++) {
            ...
            if (isOverlapping) {
                if (isSamePlace) {
                    ...
                    let severity: TripWarning['severity'] = isBothBooked ? 'critical' : 'warning';
...
export function validateAccommodationGaps(trip: Trip, warnings: TripWarning[], style?: TravelStyle) {
    ...
    while (currentDate < end) {
        ...
        if (!isCovered) {
            ...
            if (style?.planning === 'planned') severity = 'warning';
...
export function validateAccommodationCapacity(trip: Trip, warnings: TripWarning[]) {
    if (!trip.accommodation || !trip.participants) return;
...
export function validateAccommodationExpectedTimes(trip: Trip, warnings: TripWarning[], style?: TravelStyle) {
    (trip.accommodation || []).forEach(acc => {
        // 체크인 시간 체크
        if (acc.expectedCheckInTime && acc.checkInStartTime) {
            const expected = timeToMinutes(acc.expectedCheckInTime);
            const policy = timeToMinutes(acc.checkInStartTime);
        ...
        if (acc.expectedCheckOutTime && acc.checkOutEndTime) {
            const expected = timeToMinutes(acc.expectedCheckOutTime);
            const policy = timeToMinutes(acc.checkOutEndTime);
...
export function checkAccommodationFlightConflict(trip: Trip, warnings: TripWarning[]) {
    if (!trip.accommodation || !trip.flights) return;
    ...
    trip.accommodation.forEach(acc => {
        ...
        trip.flights?.forEach(flight => {
            ...
            if (flight.date === acc.startDate && flight.arrivalTime) {
                const arrTime = timeToMinutes(flight.arrivalTime)!;
                const checkIn = timeToMinutes(acc.expectedCheckInTime || acc.checkInStartTime || '15:00')!;
            ...
            if (flight.date === acc.endDate && flight.departureTime) {
                const depTime = timeToMinutes(flight.departureTime)!;
                const checkOut = timeToMinutes(acc.expectedCheckOutTime || acc.checkOutEndTime || '11:00')!;
...
export function validateNoAccommodation(trip: Trip, warnings: TripWarning[]) {
...
export function validateCheckoutDaySchedule(trip: Trip, warnings: TripWarning[]) {
    if (!trip.accommodation || !trip.dailyTimeline) return;

    trip.accommodation.forEach(acc => {
        if (!acc.endDate) return;
        const checkOutEndTime = acc.expectedCheckOutTime || acc.checkOutEndTime || '11:00';
        const checkOutMinutes = timeToMinutes(checkOutEndTime);
        if (checkOutMinutes === null) return;

        // 체크아웃 날짜에 해당하는 일정 찾기
        const checkoutDay = trip.dailyTimeline?.find(day => day.date === acc.endDate);
        if (!checkoutDay?.events || checkoutDay.events.length === 0) return;
