...
export function validateInternationalLicense(trip: Trip, warnings: TripWarning[]) {
    const hasDriving = (trip.driving || []).length > 0;
    const isOverseas = trip.isOverseas || trip.flights?.some(f => f.isInternational);
    if (!hasDriving || !isOverseas) return;

    // 체크리스트에서 국제면허 관련 항목 검색
    const hasLicenseChecklist = trip.checklist?.some(item =>
        item.title.includes('국제면허') || 
...
export function validateHealthPreparation(trip: Trip, warnings: TripWarning[]) {
    const isOverseas = trip.isOverseas || trip.flights?.some(f => f.isInternational);
    if (!isOverseas) return;
    if (!trip.dates?.durationDays || trip.dates.durationDays < 5) return; // 5일 이상 장기 여행만

    const hasHealthChecklist = trip.checklist?.some(item =>
        item.title.includes('약') || 
...
export function validateCommunicationPrep(trip: Trip, warnings: TripWarning[]) {
    const isOverseas = trip.isOverseas || trip.flights?.some(f => f.isInternational);
    if (!isOverseas) return;

    const hasCommChecklist = trip.checklist?.some(item =>
        item.title.includes('로밍') || 
...
export function validateAirportTransfer(trip: Trip, warnings: TripWarning[]) {
    if (!trip.flights || !trip.dailyTimeline) return;

    // 도착 항공편 찾기
    const arrivalFlights = trip.flights.filter(f => 
        f.type === 'outbound' && f.date && f.arrivalTime
    );
    if (arrivalFlights.length === 0) return;
    ...
    arrivalFlights.forEach(flight => {
        ...
        const hasTransportAfterArrival = arrivalDay.events?.some(e => {
            if (!e.startTime || !flight.arrivalTime) return false;
            // 도착 후의 이동 관련 이벤트
            return e.startTime >= flight.arrivalTime && (
...
export function validateLateNightReturn(trip: Trip, warnings: TripWarning[]) {
    (trip.dailyTimeline || []).forEach((day, dayIdx) => {
        ...
        const hasAccommodation = (trip.accommodation || []).some(acc => 
            acc.startDate && acc.endDate && dayDate && dayDate >= acc.startDate && dayDate < acc.endDate
        );
