import { Trip, TripDocument, TripEvent } from '../types/trip';
import { getUserTrips, getTrip } from './tripService';
import { getUserTravelogs } from './recordService';

export interface UserStats {
    totalTrips: number;
    totalDays: number;
    visitedRegions: { name: string; count: number }[];
    stayStats: {
        totalNights: number;
 ...
 */
export const aggregateUserStats = async (userId: string): Promise<UserStats> => {
    try {
        const summaries = await getUserTrips(userId);
        const travelogs = await getUserTravelogs(userId);
        
        // 각 여행의 상세 데이터를 가져와서 집계 (병렬 처리, 개별 오류 방지)
        const detailedTrips = await Promise.all(
            summaries.map(async (s) => {
                try {
                    return await getTrip(s.id);
                } catch (e) {
