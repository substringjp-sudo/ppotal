...
const REGION_THEMES: Record<string, ThemeOption> = {
  ...
  'island': {
    ...
    silhouetteColor: '#FFFFFF',
    moodImageUrl: MOOD_IMAGES.ISLAND
  },
  'city': {
    background: '#475569',
    gradient: 'from-slate-700 via-slate-800 to-slate-950',
    silhouetteColor: '#60A5FA',
    moodImageUrl: MOOD_IMAGES.CITY
  },
  'nature': {
    background: '#15803D',
    gradient: 'from-green-600 via-emerald-700 to-slate-900',
    silhouetteColor: '#FFFFFF',
    moodImageUrl: MOOD_IMAGES.NATURE
  }
};

const DEFAULT_THEME: ThemeOption = {
  background: DESIGN_TOKENS.colors.primary.DEFAULT,
  gradient: 'from-primary/80 via-indigo-600 to-purple-700',
  silhouetteColor: '#FFFFFF'
};

export function getTripTheme(regions: TripRegion[]): ThemeOption {
  if (!regions || regions.length === 0) return DEFAULT_THEME;
  ...
  for (const region of regions) {
    ...
    if (REGION_THEMES[name]) return REGION_THEMES[name];
    
    // Check aliases
    if (name === 'south korea' || name === 'korea') return REGION_THEMES['korea south'];
    if (name === 'usa' || name === 'united states') return REGION_THEMES['united states of america'];
    if (name === 'uk') return REGION_THEMES['united kingdom'];
  }

  // 2. Try to match by keywords
  for (const region of regions) {
    const name = normalize(region.name);
    if (name.includes('island') || name.includes('bali') || name.includes('hawaii') || name.includes('okinawa')) return REGION_THEMES['island'];
    if (name.includes('mountain') || name.includes('park') || name.includes('nature')) return REGION_THEMES['nature'];
    if (name.includes('new york') || name.includes('tokyo') || name.includes('seoul') || name.includes('beach')) {
      if (name.includes('beach')) return REGION_THEMES['island'];
